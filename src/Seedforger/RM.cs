...
namespace Seedforger {
  internal partial class RM : UserControl {
    ...
    internal RM() {
      InitializeComponent();
      foreach (var f in TorrentClientFactory.GetFamilies()) {
        cmbClient.Items.Add(f);
      }
    ...
    internal void cmbClient_SelectedIndexChanged(object sender, EventArgs e) {
      ...
      var family = cmbClient.SelectedItem?.ToString();
      ...
      if (customPeersNum.Text == "0" || customPeersNum.Text == "") {
        customPeersNum.Text = TorrentClientFactory.GetDefaultNumWant(family).ToString();
      }
    ...
    internal void StartButton_Click(object sender, EventArgs e) {
      ...
      Leechers = -1;
      if (trackerAddress.Text == "" || shaHash.Text == "" || txtTorrentSize.Text == "") {
        MessageBox.Show("Please select valid torrent file!", "Seedforger " + version + " - ERROR");
        return;
      }
    ...
    // Both the HTTP (proxy-aware) and HTTPS transports now live in the WinForms-free
    // Seedforger.Core (TrackerTransport), shared with the headless engine. Behaviour
    // is identical — only the log sink is passed in.
    private TrackerResponse MakeWebRequestEx(Uri reqUri) =>
      TrackerTransport.Fetch(reqUri, currentClient, currentProxy, AddLogLine);
    ...
    internal bool GetData(string client, string pVersion, string searchString, long startOffset, long maxOffset) {
      try {
        ...
        while (currentOffset < maxOffset) {
          ...
          // pReader.saveArrayToFile(buffer1, @"D:\Projects\NRPG Ratio\NRPG Seedforger MULTIPLE\Seedforger source\bin\Release\tests\test" + currentOffset.ToString() + ".txt");
