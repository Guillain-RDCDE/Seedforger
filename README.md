...
The engine core is WinForms-free (`Seedforger.Core`), so the headless CLI
(`Seedforger.Cli`) builds and runs on **Linux, macOS and Windows** — ideal for a
seedbox, a server or CI. It ships as a self-contained single file (no runtime to
install). Full reference in [Command line](docs/cli.md).
